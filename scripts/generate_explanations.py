...
【解説】
なぜその選択肢が正答（または最も不適切）なのかを1〜3文で説明する。
核心だけを述べる。専門用語は使ってよい。
文体は体言止めまたは「〜である。」で統一する。

【参考】
問題の具体的なテーマに直接関係するURLを1〜2件記載する。
URLの直前の行に「何についてのページか」が分かる説明文を書く。

禁止事項：
- 「【誤りのポイント】」「【各選択肢の解説】」などの追加セクション（3セクションのみ）
- 各選択肢（①〜⑤）の個別列挙・解説
- 「適切な記述である」「正しい記述である」「適切です」「正しいです」等の確認フレーズ
- 文末の「〜ており。」「〜あり。」等の中途半端な文体
- 同じ内容の繰り返し

【参考】セクションのルール：
- 大カテゴリページや林野庁トップページは不可。問題のテーマに直結する具体的なページのみ。
- 確実にアクセスできるURLのみ（404・リダイレクトは不可）
- 複数の問題で同じURLを使い回さない
- URLがない場合はこのセクションを丸ごと省略する（書誌情報のみの記載は不要）"""
...
def main():
    ...
    parser.add_argument("--model", default="claude-sonnet-4-6", help="使用モデル（デフォルト: claude-sonnet-4-6）")
